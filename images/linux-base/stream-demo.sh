...
sleep 0.5
cd /src
cargo build -q --release -p iapetusd --features x11 -p iapetus-gateway

IAPETUS_GATEWAY_BIND=0.0.0.0:8080 target/release/iapetus-gateway &
sleep 1
...
exec target/release/iapetusd --stream "ws://127.0.0.1:8080/ingest?token=dev"
